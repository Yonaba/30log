...
local baseMt, _instances, _classes, _class, class = {}, setmetatable({},{__mode='k'}), setmetatable({},{__mode='k'})
local function assert_class(class, method) assert(_classes[class], ('Wrong method call. Expected class:%s.'):format(method)) end
local function deep_copy(t, dest, aType, n) t = t or {}; local r = dest or {}
  for k,v in pairs(t) do if aType and type(v)==aType then r[k] = v
    elseif not aType then if type(v) == 'table' and k ~= "__index" then if _instances[v] or _classes[v] then r[k] = class.clone(n, v) else r[k] = deep_copy(v, nil, aType, n) end
      else r[k] = v end end end ; return r
end
...
local function extend(self, name, extra_params)
  assert_class(self, 'extend(...)'); local heir = {}; _classes[heir] = tostring(heir); deep_copy(extra_params, deep_copy(self, heir,nil, name), nil, name);
...
class = {isClass = function(class, ofsuper) local isclass = not not _classes[class]; if ofsuper then return isclass and (class.super == ofsuper) end; return isclass end, isInstance = function(instance, ofclass) 
	local isinstance = not not _instances[instance]; if ofclass then return isinstance and (instance.class == ofclass) end; return isinstance end, clone = function(name, tbl) assert(_classes[tbl] or _instances[tbl], 'Wrong argument #2. Expected a class or an instance')
		if _classes[tbl] then local super = tbl.super; local copy = super and super:extend(name, tbl) or _class(name, tbl); for mixin in pairs(tbl.mixins) do copy:include(mixin) end; return copy end; return deep_copy(tbl, tbl.class:new(), nil, name)
  end}; _class = function(name, attr)
  ...
  c.extends = function(self, class) assert_class(self, 'extends(class)') local super = self; repeat super = super.super until (super == class or super == nil); return class and (super == class) end
	return setmetatable(c, baseMt) end; class._DESCRIPTION = '30 lines library for object orientation in Lua'; class._VERSION = '30log v1.1.0'; class._URL = 'http://github.com/Yonaba/30log'; class._LICENSE = 'MIT LICENSE <http://www.opensource.org/licenses/mit-license.php>'
