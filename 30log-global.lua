...
local function deep_copy(t, dest, aType) t = t or {}; local r = dest or {}; for k,v in pairs(t) do if aType ~= nil and type(v) == aType then r[k] = (type(v) == 'table') and deep_copy(v) or v elseif aType == nil then r[k] = (type(v) == 'table') and k~= '__index' and deep_copy(v) or v end; end return r end
...
_class = function(name, attr) local c = deep_copy(attr); _classes[c] = tostring(c)
	...
	c.without = function(self, ...) assert_call_from_class(self, 'without(mixin)'); for _, mixin in ipairs({...}) do
		...
		for _, class in ipairs(classes) do for method_name, method in pairs(mixin) do if type(method) == 'function' then class[method_name] = nil end end end
		self.mixins[mixin] = nil end; return self end; return setmetatable(c, baseMt) end
